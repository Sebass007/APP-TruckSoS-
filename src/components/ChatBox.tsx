...
export default function ChatBox({ solicitudId, currentUserId, onClose, tipoUsuario }: ChatProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-neutral-900 border border-neutral-800 rounded-3xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4">
      {/* Header */}
      <div className="p-3 sm:p-4 bg-neutral-800/95 backdrop-blur-md border-b border-neutral-700 flex justify-between items-center shrink-0 gap-2 overflow-hidden">
        <div className="flex items-center gap-2 min-w-0 shrink">
          <div className="w-7 h-7 sm:w-8 sm:h-8 bg-orange-600 rounded-full flex items-center justify-center shadow-lg shadow-orange-600/30 shrink-0">
            <User className="w-4 h-4 text-white" />
          </div>
          <div className="min-w-0">
            <p className="text-[10px] sm:text-xs font-black italic uppercase text-white tracking-wider truncate">Chat Negociación</p>
            <p className="text-[8px] sm:text-[9px] text-green-400 font-extrabold uppercase flex items-center gap-1">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {tipoUsuario === 'proveedor' && (
            <button 
              ...
              className="bg-green-600 hover:bg-green-500 text-white px-2.5 py-1.5 rounded-xl font-black text-[9px] sm:text-[10px] uppercase italic transition-all flex items-center gap-1 shadow-md shadow-green-600/30 shrink-0"
            >
          ...
          <button 
            onClick={onClose} 
            className="p-1.5 sm:p-2 bg-neutral-700/80 hover:bg-neutral-700 text-white rounded-xl border border-neutral-600 transition-all shadow-md shrink-0"
            title="Cerrar Chat"
          >
